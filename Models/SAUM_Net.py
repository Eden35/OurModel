...
from .mfii_resnet import  MFII_resnet34_S
...
class SAUM_Net(nn.Module):
    def __init__(self, classes=2, channels=3):
        super(SAUM_Net, self).__init__()
        ...
        self.flat_layer2 = HIFA_V2(in_channels=256+16, ratio= 2)
        self.drop_block2 = Drop(drop_rate=0.2, block_size=2)
        ...
        self.decoder2_4 = MFII_DecoderBlock_V2(256, filters[2], rla_channel=self.rla_channel, ECA_size=7)
        self.decoder2_3 = MFII_DecoderBlock_V2(filters[2], filters[1], rla_channel=self.rla_channel, ECA_size=5)
        self.decoder2_2 = MFII_DecoderBlock_V2(filters[1], filters[0], rla_channel=self.rla_channel, ECA_size=5)
        self.decoder2_1 = MFII_DecoderBlock_V2(filters[0], filters[0], rla_channel=self.rla_channel, ECA_size=5)
        ...
        self.finaldecon2_v1 = nn.ConvTranspose2d(filters[0] + self.rla_channel, 32, 4, 2, 1)
        self.finalrelu2_1 = nonlinearity
        self.finalconv2_3 = nn.Conv2d(32, classes, 3, padding=1)
    ...
    def forward(self, x,LFAT_pre):
        ...
        e1, e2, e3, e4, e_h1, e_h2, e_h3, e_h4, e2_1, e2_2, e2_3, e2_4 = self.model(x,LFAT_pre)
        # Center

        flat_feature = torch.cat((e4, e_h4), dim=1)
        flat_feature = self.flat_layer(flat_feature)
        flat_feature = self.drop_block(flat_feature)
        e4_flat, eh4_flat1 = torch.split(flat_feature, [256, 16], dim=1)

        flat_feature2 = torch.cat((e2_4, e_h4), dim=1)
        flat_feature2 = self.flat_layer2(flat_feature2)
        flat_feature2 = self.drop_block2(flat_feature2)
        e2_4_flat, eh4_flat2 = torch.split(flat_feature2, [256, 16], dim=1)

        eh4_flat = eh4_flat1+eh4_flat2
        ...
        d2_3, dh2_3 = self.decoder2_4(e2_4_flat, dh_4)
        d2_3= d2_3 + e2_3
        ...
        d2_2, dh2_2 = self.decoder2_3(d2_3, dh_3)
        d2_2 = d2_2 + e2_2
        ...
        d2_1, dh2_1 = self.decoder2_2(d2_2, dh_2)
        d2_1 = d2_1 + e2_1
        ...
        d2_0, dh2_0 = self.decoder2_1(d2_1, dh_1)

        d2_0_out = torch.cat((d2_0, dh2_0), dim=1)

        out = self.finaldeconv1(d0_out)
        out = self.finalrelu1(out)
        out = self.finalconv3(out)   

        out2 = self.finaldecon2_v1(d2_0_out)
        out2 = self.finalrelu2_1(out2)
        out2 = self.finalconv2_3(out2)
        # return F.sigmoid(out),F.sigmoid(out2)
        return out, out2
