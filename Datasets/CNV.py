...
class CNV_dataset(Dataset):
    def __init__(self, dataset_folder='/home/data/project2/demo2/data/CNV',
        ...
        if self.train_type in ['train', 'validation', 'test']:
            ...
            self.mask = [join(dataset_folder, 'label', x.split('.')[0] + '.npy') for x in self.image_list]
    ...
    def __getitem__(self, item: int):
        ...
        img_path = self.folder[item]

        msk_path = self.mask[item]
        if not os.path.isfile(img_path):
            raise FileNotFoundError(f"Image file {img_path} not found")
        if not os.path.isfile(msk_path):
            raise FileNotFoundError(f"Mask file {msk_path} not found")

        image = np.load(img_path)
        label = np.load(msk_path)

        if image is None or label is None:
            raise ValueError(f"Loaded image or label from {img_path} or {msk_path} is None")
...
class CNV_dataset_with_name(Dataset):
    def __init__(self, dataset_folder='/home/data/project2/demo2/data/CNV',
